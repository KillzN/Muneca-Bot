  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        // credit to josselyne🩷 or skidy89
        const dick = {
          afk: -1,
          wait: 0,
          afkReason: '',
    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          fetch('https://thejosselyne🩷bot.cloud:2083/error', {
            method: 'POST',
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/KillzN/Muneca-Bot/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;josselyne🩷 👑;;;\nFN:josselyne🩷 👑\nORG:josselyne🩷 👑\nTITLE:\nitem1.TEL;waid=50766454639‬:+50766454639‬\nitem1.X-ABLabel:josselyne🩷 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:josselyne🩷 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'josselyne🩷 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
